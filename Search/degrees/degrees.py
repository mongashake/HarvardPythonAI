...
def shortest_path(source, target):
    ...
    while True:
        ...
        target_node = expand_node(frontier, target, explored)
        if target_node:
            break
...
def expand_node(frontier, target, explored):

    person = frontier.remove()

    for neighbor in neighbors_for_person(person.state):
        actor = Node(neighbor[1], person, neighbor[0])
        if actor.state == target:
            return actor

        if not frontier.contains_state(actor.state) and actor.state not in explored:
            frontier.add(actor)
